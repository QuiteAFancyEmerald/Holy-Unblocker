...
module.exports = function setupRoutes(proxyServer, sessionStore, logger) {
    ...
    const autocompleteId = 'collectsearchautocompleteresults';

    if (!sessionStore.has(autocompleteId)) {
        const session = new RammerheadSession({id: autocompleteId, dontConnectToData: true});
        // workaround for saving the modified session to disk
        sessionStore.addSerializedSession(autocompleteId, session.serializeSession());
    }
};
